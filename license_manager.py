...
def run_action(store, command):
    ...
    if command == '3':
        ...
        for event in reversed(store.history()):
            before_rows = {r['licenseId']:r for r in upgrade(event['before'])['licenses']}
            after_ids = {r['licenseId'] for r in event['after']['licenses']}
            if ident in before_rows and ident not in after_ids:
                found = copy.deepcopy(before_rows[ident])
                break
        if not found:
